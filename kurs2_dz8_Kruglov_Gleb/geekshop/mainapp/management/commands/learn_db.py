...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        #     Q(category__name__icontains='Stealth Games') |
        #     Q(category__name__icontains='Role-Playing Games')
        # ).select_related()

        # test_products = Product.objects.filter(
        #     Q(price__lte=10) & Q(name__icontains='f') &
        #     (Q(category__name__icontains='Simulator Games') |
        #     Q(category__name__icontains='Role-Playing Games'))
        # ).select_related()
        #
